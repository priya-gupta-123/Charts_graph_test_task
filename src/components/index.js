...
export default class index extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.selectedTable !== selectedTable) {
      if (
        selectedTable.includes("Haryana") ||
        selectedTable.includes("Punjab") ||
        selectedTable.includes("Himachal")
      ) {
        const rowData = CollegeFakeDb.data.filter(
          (data) => data.State === selectedTable
        );
        return this.setState({ ActiveTable: CollegeTable, rowData });
      } else {
        const rowData = CollegeFakeDb.data.filter((data) =>
          data.Courses.includes(selectedTable)
        );
        return this.setState({ ActiveTable: CollegeTable, rowData });
      }
    }
